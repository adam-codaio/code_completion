...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        gradients, values = zip(*gvs)# = [grad for grad, val in gvs]
        if self.config.clip_gradients:
            gradients, grad_norm = tf.clip_by_global_norm(gradients, clip_norm=self.config.max_grad_norm, use_norm = self.grad_norm)
	    #new_gvs = []
	    #for i in range(len(gvs)):
	    #    new_gvs.append((gradients[i], gvs[i][1]))
	    #gvs = new_gvs
