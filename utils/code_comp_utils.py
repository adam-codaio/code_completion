...
def process_token_list(token_list, non_terminal):
    '''
    Takes the token_list and separates it into features and label for each
    segment.
    '''
    token_list = token_list[:-1] # Drop last token which is always program
    segments = []
    for i in xrange(0, len(token_list), config.segment_size):
        ...
        idx = 0 if non_terminal else 1
        label = [token_list[-1][idx]]
        segments.append([features, label])
    return segments

def read_json(infile, reduced=False, num_examples=None, non_terminal=True):
    '''
    ...
    with open(infile) as f:
        for line in f:
            ...
            segments = process_token_list(token_list, non_terminal)
